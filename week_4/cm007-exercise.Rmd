...
gapminder %>% 
  ...
  drop_na() %>% 
  ungroup() %>% 
  # first phase ends here (calculating increase in lifeExp by country and create a new column inc_life_exp).
  group_by(continent) %>% 
  filter(inc_life_exp == min(inc_life_exp)) %>% 
  arrange(inc_life_exp) %>% 
  # second phase ends here (in each continent, present the country with the lowest inc_life_exp).
  knitr::kable()
