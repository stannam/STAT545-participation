...
```r
gapminder %>% 
  # group_by(country) %>%  #ggplot ignores groups in a tibble. so comment out this line
  ggplot(aes(year, lifeExp, group = country, colour = country == "Rwanda")) + 
  #instead give group in the global aes, give color to Rwanda only. It makes a new column of TRUE and FALSE on the fly. 
  geom_line(alpha = 1/3) +
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  #labels argument is the thing you need when you try to do something to the labels. labels value of scales::comma_format() do this
  facet_wrap(~ continent, scales = "free_y") # each panel for each continent
```
