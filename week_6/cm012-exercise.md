...
library(tidyverse)
library(gapminder)
library(forcats) # the 'forcats' package is part of tidyverse so this line is redundant. :p
```
...
gapminder %>% 
  filter(year == 1997) %>% 
  mutate(life_level = factor(case_when(lifeExp < 23 ~ 'very low',    # looks better when aligned. :)
                                       lifeExp < 48 ~ 'low',
                                       lifeExp < 59 ~ 'moderate',
                                       lifeExp < 70 ~ 'high',
                                       TRUE ~ 'very high'),
...
- what class is  `continent`(a factor or charecter)? it is a 'factor'
- How many levels? What are they? 5 - They are "Africa", "Americas", "Asia", "Europe" and "Oceania"
- What integer is used to represent factor "Asia"? 3


```r
class(gapminder$continent) # the class of 'continent' is factor
```
...
```r
# same result with piping

gapminder$continent %>%   # this time, I used piping. 
  nlevels()               # nlevels returns the number of levels.
```
...
```r
gapminder$continent %>%  # this returns.. Factor w/ 5 levels "Africa","Americas",..: 3 3 3 3 3 3 3 3 3 3 ...
  str()
```
...
```r
levels(gapminder$continent)  # The levels are... 
```
...
- Filter `gapminder` dataset by 5 countries of your choice. How many levels are in your filtered dataset? Still 142.


```r
nlevels(gapminder$country) # How many levels are there in `country`? 142
```
...
```r
h_countries <- c("China", "Taiwan", "Japan", "Korea, Dem. Rep.", "Korea, Rep.")

gap <- gapminder %>%
  filter(country %in% h_countries)

gap$country %>% 
  nlevels()   # The filtered dataset still has 142 levels
```
